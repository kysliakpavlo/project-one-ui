...
import { removeNumber, dynamicFilter, filterToChips, scrollToTop, filterKeyMap, setAppTitle, setSortObj, setBooleanFromString } from '../../../utils/helpers';

import './AuctionsView.scss';

const pages = [{ label: 'Home', path: '/' }];

class AuctionsView extends Component {
	constructor(props) {
		super(props);
		const {
			history: { action },
		...
		this.state = {
			...
			searchCrietria: {},
			filters: [],
			filteredObj: isBack ? filtersState.filteredObj || {} : location.state?.filteredObj || {},
			sortPaging:
				isBack && !_isEmpty(filtersState.sortPaging)
					? filtersState.sortPaging
					: {
							sortBy: location.state?.savedSearch ? 'createdDate/desc' : 'lotNo',
							pageSize: 12,
							activePage: 0,
					  },
			activeView: isBack && filtersState.activeView ? filtersState.activeView : 'grid',
			refineSearch: false,
			isFilter: false,
	...
	componentDidMount() {
		...
		const { view, activePage, pageSize, filters } = this.state;
		...
		if (filterobj.type && filterobj.type === 'edit') {
			...
			this.setState({ savedSearch: { label: 'Save this Search', id: null } });
		}
		...
		if (view === 'featured') {
			this.getFeaturedList(req);
		} else if (view === 'featuredAssets') {
			this.getFeaturedAssets(req);
		} else if (view === 'featuredAuctions') {
			this.getSortByAuctions({ featuredAuc: 1, ...req });
		} else {
			req.sortBy = view;
			this.getSortByAuctions(req);
		}
	}

	getFeaturedList = (req) => {
		const { setLoading, getFeatured } = this.props;
		setLoading(true);
		getFeatured({ ...req, type: 'asset,auction' })
			.then((response) => {
				// this.setFeaturedList(response);
				this.setState({ featuredList: response });
				setLoading(false);
				this.scrollToCard();
			})
			.catch((e) => {
	...
	scrollToCard = () => {
		const { detailsAuction, setDetailsAuction } = this.props;
		if (detailsAuction) {
			setTimeout(() => {
				const auctionEle = document.querySelector(`#auction_card_${detailsAuction}`);
				if (auctionEle) {
	...
	getFeaturedAssets = (req) => {
		const { setLoading, getFeatured } = this.props;
		setLoading(true);
		getFeatured({ ...req, type: 'asset' })
			.then((res) => {
				// setFeaturedAssets(res);
				this.setState({ featuredAssets: res });
				setLoading(false);
				this.scrollToCard();
			})
			.catch((e) => {
	...
	getSortByAuctions = (req) => {
		const { setLoading, getAuctions } = this.props;
		setLoading(true);
		getAuctions(req)
			.then((res) => {
				setLoading(false);
	...
	render() {
		const { socket, location, showMessage, loggedInUser, toggleLogin, history, isLoading, isLoggedIn } = this.props;
		const { searchResults, filters, filteredObj, sortPaging, activeView, searchCrietria, refineSearch, saveSearchLabel, savedSearch, sortAuctions } = this.state;

		const { search } = location;
		const filterobj = parse(this.props.location?.search.split('?')[1]);
		const groupName = filterobj.title;
		// TODO: throws an error investigate and redo
				...
				<Container className="search-view search-result-view">
					<Visible when={searchResults}>
						...
						<div className="search-result-view-content">
							...
							{_map(sortAuctions.result, (auction) => {
								return (
									<div
							...
							<Visible when={!isLoading && _isEmpty(sortAuctions.result)}>
								<NoResults />
							</Visible>
						</div>
					</Visible>
